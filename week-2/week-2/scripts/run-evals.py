...
to headless Claude Code (`claude -p "..."`). No Anthropic API key required:
Claude Code handles auth, we just pipe prompts through it and collect scores.

The pattern is simple enough to teach and powerful enough to be real:

    for each test case:
        1. call `claude -p <product_prompt + input_data>` -> feature output
        2. call `claude -p <judge_prompt + output + rubric>` -> JSON scores
...
Usage:
    ...
    python3 run-evals.py                     # run with defaults
...
# Claude Code runs a default model under the hood; we don't pick one here.
# If you want to pin a model, set it via Claude Code itself (`claude /model`).
DEFAULT_TIMEOUT = 120
DEFAULT_PARALLEL = 4
...
def build_judge_prompt(case: dict, product_output: str, rubric: list) -> str:
    ...
    for r in rubric:
        criteria_lines.append(
            ...
            f"5/5 = {r['five_out_of_five']} | 1/5 = {r['one_out_of_five']}"
        )
...
def main():
    ...
    with cf.ThreadPoolExecutor(max_workers=max(1, args.parallel)) as pool:
        futures = {
            pool.submit(score_case, c, product_prompt, rubric, args.timeout): c
            for c in cases
        }
